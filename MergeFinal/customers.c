...
static customers_balance(void){
    ...
    while (!finished && (fgets(x1, (int) sizeof(x1), stdin) != NULL)) {  
        char query[BufferLength + 100];
        /* snprintf is not defined if ansi flag is enabled */
        (void) snprintf(query, (size_t)(BufferLength + 150), "SELECT(SELECT SUM(amount) AS saldo FROM payments WHERE customernumber = %d)-(SELECT SUM(od.quantityordered * od.priceeach) AS buyed FROM customers c NATURAL JOIN orders NATURAL JOIN orderdetails od WHERE c.customernumber = %d) as risultato;",atoi(x1),atoi(x1));
        (void) SQLExecDirect(stmt, (SQLCHAR*) query, SQL_NTS);
        /* Vincula las columnas de la consulta a variables */
        SQLBindCol(stmt, 1, SQL_C_CHAR, risultato, BufferLength, NULL);
